...
def main(_):
    ...
    if FLAGS.dataset_name == 'DAVIS':
        if FLAGS.use_focal_loss:
            ...
            wl = tf.where(tf.logical_and(tf.greater((b_trimap+128),1), tf.less((b_trimap+128), 200)),
                            tf.fill([train_batch_size,image_width,image_height,1],1.),
           ...
           #                     tf.fill([train_batch_size, image_width, image_height, 1], 1.),\
           #                     tf.fill([train_batch_size, image_width, image_height, 1], 0.))
            wl = tf.where(tf.logical_and(tf.greater((b_trimap+128), 50), tf.less((b_trimap+128), 200)),
                            tf.fill([train_batch_size,image_width,image_height,1],1.),
        ...
        if FLAGS.use_focal_loss:
            print 'using focal loss'
            wl = tf.where(tf.equal((b_trimap+128),128), tf.fill([train_batch_size,image_width,image_height,1],.5), tf.fill([train_batch_size,image_width,image_height,1], 0.))
        else:
            wl = tf.where(tf.equal((b_trimap+128),128), tf.fill([train_batch_size,image_width,image_height,1],1.), tf.fill([train_batch_size,image_width,image_height,1], 0.))
