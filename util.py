...
def input_parser(image_name, dataset_alpha, dataset_trimap, 
    ...
    BG_path = os.path.join(dataset_bg, image_name)
    RGB_path = os.path.join(dataset_RGB, image_name)
    image_stack = load_single_image_DAVIS(alpha_path, trimap_path,
                                          FG_path, BG_path, RGB_path)
    if is_training:
        image_stack = image_preprocessing(img_stack, True)
    return image_stack
